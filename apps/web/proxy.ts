...
const USER_ROUTES = ["/kaydettiklerim", "/alarmlar", "/gecmis"];
 ...
 *
 * 1. `/yonetim/*`, `/kaydettiklerim`, `/alarmlar`, `/gecmis`: yalnızca
 *    iyimser kontrol - `session` çerezi yoksa `/giris`'e yönlendirir.
 *    Veritabanına gitmez, rolü doğrulamaz - gerçek kontrol `app/lib/
 *    dal.ts`'teki `requireRole`/`requireUser` her sayfa ve server action'da
 *    tekrar yapılır (Next'in kendi rehberi: proxy tek başına yeterli değil).
...
export function proxy(request: NextRequest): NextResponse {
  ...
  const needsSession = pathname.startsWith("/yonetim") || USER_ROUTES.includes(pathname);
...
export const config = {
  matcher: ["/yonetim/:path*", "/ara/:path*", "/kaydettiklerim", "/alarmlar", "/gecmis"],
};
